...
<div x-data="{
    ...
    x-on:keydown.tab.prevent="
        if (!$event.shiftKey) {
            if (!focusableElements.includes(document.activeElement) || document.activeElement === focusableElements[focusableElements.length - 1]) {
                focusableElements[0].focus();
            } else {
                focusableElements[focusableElements.indexOf(document.activeElement) + 1].focus();
            }
        } else {
            if (!focusableElements.includes(document.activeElement) || document.activeElement === focusableElements[0]) {
    ...
    "
    x-show="show" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" style="display: none;">
